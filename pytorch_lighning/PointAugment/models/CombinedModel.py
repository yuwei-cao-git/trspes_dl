...
from common.opt_and_schedulars import (
    get_optimizer_c,
    get_optimizer_a,
    get_lr_scheduler,
    get_lr_scheduler_step,
)
...
class CombinedModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_augmentor:
            ...
            opt_c.zero_grad()
            self.log_dict(
                {"loss_classifier": loss_classifier, "loss_augmentor": loss_augmentor},
                prog_bar=True,
            )
            train_r2_score = r2_score(
                F.softmax(logits_data, dim=1).flatten().round(decimals=2),
                target.flatten(),
            )
            aug_r2_score = r2_score(
                F.softmax(logits_aug_data, dim=1).flatten().round(decimals=2),
                target.flatten(),
            )
            self.log_dict(
                {"train_r2_score": train_r2_score, "train_aug_r2_score": aug_r2_score},
                prog_bar=True,
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.validation_step_outputs.append(
            {"val_loss": loss, "val_target": target, "val_pred": preds}
        )
        self.log(
            "val_loss", loss, prog_bar=True, logger=True, sync_dist=True
        )  # TODO: on_step or on_epoch is needed
        return {"val_loss": loss}

    def on_validation_epoch_end(self):
        last_epoch_val_loss = torch.mean(
            torch.stack([output["val_loss"] for output in self.validation_step_outputs])
        )
        ...
        if last_epoch_val_loss > self.best_test_loss:
            ...
            test_true = (
                torch.stack(
                    [output["val_target"] for output in self.validation_step_outputs]
                )
            ).flatten()
            test_pred = (
                (
                    torch.stack(
                        [output["val_pred"] for output in self.validation_step_outputs]
                    )
                )
                .flatten()
                .round(decimals=2)
